    ...
    <table class="table table-bordered sortable">
        ...
        @foreach ( $categories as $category )
            <tr>
                ...
                <td>
                    @if(!$trashed)
                        <a href="{{ route('admin.categories.edit', $category ) }}"><span class="fa fa-edit fa-fw"></span></a>
                        {!! Form::open( [ 'route' => ['admin.categories.destroy', $category], 'class' => $category->id . ' form-delete' ] ) !!}

                        {!! Form::hidden('_method', 'delete') !!}
                            <a href="#" title="{{$category->name}}" class="delete" id="{{$category->id}}"><span class="fa fa-trash-o fa-fw"></span></a>
                        {!! Form::close() !!}
                    @else
                        <a href="{{route('admin.categories.restore', $category)}}" title="">Restore</a>
                    @endif
                </td>
            </tr>
        @endforeach
